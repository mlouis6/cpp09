...
namespace jacobsthal
{
	...
	std::deque<std::size_t> getOrderD(std::size_t size)
	{
	...
	std::vector<std::size_t> getOrderV(std::size_t size)
	{
...
/** DEQUE */
std::deque<unsigned int> PmergeMe::initD(char **args, int nb_args)
{
...
std::deque<unsigned int> PmergeMe::sortFJ(const std::deque<unsigned int>& nbs)
{
	...
	std::deque<size_t> order = jacobsthal::getOrderD(pending.size());
...
std::vector<unsigned int> PmergeMe::sortFJ(const std::vector<unsigned int>& nbs)
{
	...
	std::vector<std::size_t> winnerPos;
	for (std::size_t i = 0 ; i < main.size() ; ++i)
		winnerPos.push_back(i);
	...
	std::vector<size_t> order = jacobsthal::getOrderV(pending.size());
	...
	for (std::size_t i = 0 ; i < order.size() ; ++i)
	{
		...
		std::vector<unsigned int>::iterator pos = lower_bound(main.begin(), end, pending[idx]);
